...
export default function PanelCalidad({
  ...
  useEffect(() => {
    const fetchRegistros = async () => {
      setIsLoading(true);
      try {
        const data = await registroDiarioService.getPanelCalidad();
  ...
  useEffect(() => {
    if (auditoriaFeedback) {
      setFeedback(auditoriaFeedback); // Mostramos el toast en esta pantalla
      ...
      // Limpiamos el estado global para que no vuelva a saltar si recarga la página
  ...
  const pendientes = registros.filter(
    (r) => r.auditado_calidad === false || r.auditado_calidad === 0,
  );
  const auditados = registros.filter(
    (r) => r.auditado_calidad === true || r.auditado_calidad === 1,
  );

  const handleAuditoriaGuardada = (mensaje) => {
    setFeedback({ tipo: "ok", mensaje: mensaje });
    ...
    <div className="space-y-6 animate-in fade-in duration-500 max-w-7xl mx-auto relative">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          >
            Panel de <span style={{ color: COLOR_NARANJA }}>Calidad</span>
          </h1>
