...
export default function EscogerKPI() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="animate-in fade-in duration-500">
          <div className="mb-8">
            ...
            <p className="text-gray-500 font-medium mt-1">
              Selecciona un área para definir qué indicadores se medirán esta
              semana.
            </p>
          </div>
              ...
              {areas.map((area) => (
                ...
                >
                  <div className="flex items-start justify-between mb-4 relative z-10">
                    ...
                    <div
                      className={`px-3 py-1 rounded-full text-[10px] font-black tracking-widest ${area.activos >= area.max ? "bg-green-100 text-green-700" : "bg-orange-100 text-naranja"}`}
                    >
      ...
      {selectedArea && (
        <div className="bg-white rounded-4xl shadow-sm border border-slate-100 p-6 md:p-10 animate-in slide-in-from-right-8 duration-500">
          <button
            onClick={volverAreas}
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-slate-100 pb-6 mb-6 gap-4">
            <div>
              <span className="text-[10px] bg-azul/10 text-azul px-3 py-1 rounded-full font-black uppercase tracking-widest mb-2 inline-block">
                Área Seleccionada
              </span>
              <h2 className="text-3xl font-black text-azul-profundo font-heading">
                {selectedArea.nombre}
              </h2>
            </div>
            ...
            {kpisData && (
              <div
                className={`flex items-center gap-3 px-5 py-3 rounded-2xl border ${kpisData.activos_count >= kpisData.max_activos ? "bg-green-50 border-green-200" : "bg-slate-50 border-slate-200"}`}
              >
                <Activity
                  className={`w-5 h-5 ${kpisData.activos_count >= kpisData.max_activos ? "text-green-600" : "text-slate-400"}`}
                />
                <div>
                  ...
                  <p
                    className={`text-lg font-black leading-none ${kpisData.activos_count >= kpisData.max_activos ? "text-green-700" : "text-azul-profundo"}`}
                  >
          ...
          {loadingKpis ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {kpisData.kpis.map((kpi) => (
                <div
                  key={kpi.id}
                  className={`p-5 rounded-2xl flex justify-between items-center transition-all duration-300 border ${kpi.is_programado ? "border-green-200 bg-green-50/40 shadow-sm" : "bg-white border-slate-200 hover:shadow-md hover:border-azul/30"}`}
                >
                  <div className="flex-1 pr-4">
                    <h4
                      className="font-bold text-azul-profundo text-base line-clamp-1"
                      title={kpi.nombre}
                    >
                      {kpi.nombre}
                    </h4>
                    {kpi.is_programado ? (
                      <p className="text-xs text-green-700 font-bold mt-1 flex items-center gap-1.5">
                  ...
                  {kpi.is_programado ? (
                    <span className="px-4 py-2 bg-green-100 text-green-700 font-black text-[10px] rounded-xl uppercase tracking-widest flex items-center gap-1.5 border border-green-200">
                      <CheckCircle2 className="w-4 h-4" /> Activo
                    </span>
                  ) : (
                    <button
                      onClick={() => abrirModalProgramacion(kpi)}
                      className="px-5 py-2.5 bg-slate-100 text-azul font-black text-xs rounded-xl hover:bg-azul hover:text-white transition-all shadow-sm uppercase tracking-widest"
                    >
                      Programar
                    </button>
                  )}
      ...
      {typeof window !== "undefined" &&
        createPortal(
          <>
            {/* Backdrop oscuro con z-index inmenso */}
            <div
              className={`fixed inset-0 bg-slate-900/40 z-9990 transition-opacity duration-300 ${modalSide.open ? "opacity-100" : "opacity-0 pointer-events-none"}`}
              onClick={cerrarModal}
            />

            {/* Panel lateral superpuesto */}
            <div
              className={`fixed top-0 right-0 h-dvh w-full md:w-[450px] bg-white z-9999 shadow-2xl transform transition-transform duration-300 ease-out flex flex-col ${modalSide.open ? "translate-x-0" : "translate-x-full"}`}
            >
